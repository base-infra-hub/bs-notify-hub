...
const (
	RedisSpace = "bs-notify-hub"

	// KeyUnreadPersonalPrefix  1. 【用户类】用户个人私信未读数
	KeyUnreadPersonalPrefix = RedisSpace + ":unread:personal"

	// KeyUnreadTenantPrefix 2. 【租户类】非广播的租户发来的通知未读数
	KeyUnreadTenantPrefix = RedisSpace + ":unread:tenant"

	// KeyBroadcastTotalPrefix 3. 【租户类】租户广播总集合
	KeyBroadcastTotalPrefix = RedisSpace + ":broadcast:total"

	// KeyBroadcastOpPrefix  4. 【租户类】租户下用户已操作广播集合 (已读/已删除)
	KeyBroadcastOpPrefix = RedisSpace + ":broadcast:op"
)
